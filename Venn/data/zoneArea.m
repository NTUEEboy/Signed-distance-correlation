function [Z_U,Z_D,mmm_U,mmm_D] = zoneArea(method1,method2,method3)
%% Load data
m1_U = [];
m2_U = [];
m3_U = [];
...
mmm_U = [length(m1_U) length(m2_U) length(m3_U) length(m1m2_U) length(m2m3_U) length(m1m3_U) length(m1m2m3_U)];
%% Repression
m1m2_D = {};
m2m3_D = {};
m1m3_D = {};
...
mmm_D = [length(m1_D) length(m2_D) length(m3_D) length(m1m2_D) length(m2m3_D) length(m1m3_D) length(m1m2m3_D)];
end
