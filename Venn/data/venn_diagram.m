...
for i = 1:length(sim_data)
    ...
    if auc_sdcp > auc_sdcs
        [Z_corr_U,Z_corr_D,N_corr_U, N_corr_D] = zoneArea(Pear,Spear,SDCP);
        [Z_method_U,Z_method_D,N_method_U, N_method_D] = zoneArea(SDCP,[sim_data{i},'_',method{find(r==1)},'.mat'],[sim_data{i},'_',method{find(r==2)},'.mat']);
    else
        [Z_corr_U,Z_corr_D,N_corr_U, N_corr_D] = zoneArea(Pear,Spear,SDCS);
        [Z_method_U,Z_method_D,N_method_U, N_method_D] = zoneArea(SDCS,[sim_data{i},'_',method{find(r==1)},'.mat'],[sim_data{i},'_',method{find(r==2)},'.mat']);
    end
    
    
    subplot(1,4,fig_idx)
    venn(Z_corr_U./sum(Z_corr_U))
    title('Activation (Corr)','FontSize',15)
    axis off
    fig_idx = fig_idx + 1;
    
    subplot(1,4,fig_idx)
    venn(Z_corr_D./sum(Z_corr_D))
    title('Repression (Corr)','FontSize',15)
    axis off
    fig_idx = fig_idx + 1;
    
    subplot(1,4,fig_idx)
    venn(Z_method_U./sum(Z_method_U))
    title('Activation (Method)','FontSize',15)
    axis off
    fig_idx = fig_idx + 1;
    
    subplot(1,4,fig_idx)
    venn(Z_method_D./sum(Z_method_D))
    title('Repression (Method)','FontSize',15)
    axis off
    fig_idx = fig_idx + 1;
